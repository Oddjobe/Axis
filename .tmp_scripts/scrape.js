...
async function getExistingTitles(table) {
    ...
    console.log(`  Found ${(data === null || data === void 0 ? void 0 : data.length) || 0} existing titles.`);
    return new Set((data || []).map((r) => r.title.toLowerCase().trim()));
}
async function scrapeWithPhi(url, prompt, schema, inputContent) {
    if (!foundry || !FOUNDRY_MODEL)
        throw new Error("Foundry / Phi-4 not initialized.");
    ...
    if (!content) {
        ...
        if (jinaResponse.status === 451 || !jinaResponse.ok) {
            ...
            if (!jinaResponse.ok)
                throw new Error(`Jina Reader failed: ${jinaResponse.statusText}`);
        }
...
${content.substring(0, 30000)}`;
    ...
    }, { timeout: 90000 }); // Increased timeout for parallel load
    const responseText = response.choices[0].message.content || "{}";
    const jsonMatch = responseText.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
    if (!jsonMatch)
        throw new Error("Failed to extract valid JSON from Phi response.");
    return JSON.parse(jsonMatch[0]);
}
async function scrapeIntelligence() {
    console.log(`Scraping ${INTEL_SOURCES.length} OSINT sources in parallel...`);
    const scraperPromises = INTEL_SOURCES.map(async (source) => {
        var _a, _b;
        try {
            console.log(`  [Start] ${source.name}`);
            let extracted = [];
            // 1. TRY FOUNDRY (JINA + PHI-4) - PRIMARY
            try {
                ...
                if (extracted.length > 0) {
                    console.log(`  ✓ ${source.name}: ${extracted.length} articles (Foundry)`);
                }
            }
            catch (foundryErr) {
                console.warn(`  Foundry failed for ${source.name}: ${foundryErr.message}`);
            }
            // 2. TRY FIRECRAWL - BACKUP
            if (extracted.length === 0 && firecrawl) {
                try {
                    ...
                    const extractedArticles = ((_a = response === null || response === void 0 ? void 0 : response.extract) === null || _a === void 0 ? void 0 : _a.articles) || ((_b = response === null || response === void 0 ? void 0 : response.data) === null || _b === void 0 ? void 0 : _b.articles) || (response === null || response === void 0 ? void 0 : response.articles);
            ...
            if (extracted.length > 0) {
                return extracted
                    .map((a) => (Object.assign(Object.assign({}, a), { source: source.name, isoCode: (a.isoCode || "").toUpperCase().trim(), actor: a.actor || null })))
                    .filter((a) => VALID_ISO_CODES.has(a.isoCode));
            }
        }
        catch (err) {
...
async function scrapeBlogs() {
    ...
    const scraperPromises = MEDIUM_SOURCES.map(async (url) => {
        var _a, _b;
        try {
            ...
            try {
                ...
                if (posts.length > 0) {
                    extracted = posts.map((p) => (Object.assign(Object.assign({}, p), { url: p.url || url })));
                    console.log(`  ✓ ${url}: ${extracted.length} posts (Foundry)`);
                }
            }
            catch (foundryErr) {
            ...
            if (extracted.length === 0 && firecrawl) {
                try {
                    ...
                    const extractedPosts = ((_a = response === null || response === void 0 ? void 0 : response.extract) === null || _a === void 0 ? void 0 : _a.posts) || ((_b = response === null || response === void 0 ? void 0 : response.data) === null || _b === void 0 ? void 0 : _b.posts) || (response === null || response === void 0 ? void 0 : response.posts);
                    if (Array.isArray(extractedPosts) && extractedPosts.length > 0) {
                        extracted = extractedPosts.map((p) => (Object.assign(Object.assign({}, p), { url: p.url || url })));
    ...
    const newPosts = allPosts.filter((a) => {
        const normalized = (a.title || "").toLowerCase().trim();
        return normalized.length > 0 && !existingTitles.has(normalized);
    }).map((p) => ({
        title: p.title,
...
async function runScraper() {
    console.log("Starting parallelized intelligence gathering (Foundry -> Firecrawl)...");
    await Promise.all([
        scrapeIntelligence(),
        scrapeBlogs()
    ]);
