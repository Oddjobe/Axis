...
import { BASELINE_SCORE_BY_ISO } from "./intelligence/score-methodology";
import {
    africanIso3Schema,
    deriveSovereigntyStatus,
} from "./intelligence/trust";
import dynamicNarrativesRaw from "./dynamic-narratives.json";

const dynamicNarratives = dynamicNarrativesRaw as unknown as Record<string, {
    highlights: string[],
...
const generateMockData = (): CountryData[] => {
    return AFRICAN_NATIONS.map((nation, index) => {
        const isoCode = africanIso3Schema.parse(nation.c);
        const composite = BASELINE_SCORE_BY_ISO[isoCode];
        const baseScore = composite.axisScore;
        ...
        return {
            ...
            status: deriveSovereigntyStatus(baseScore),
            population: `${(POP_MAP[nation.c] || (1 + (index * 5.3) % 40)).toFixed(1)}M`,
            resourceWealth: composite.dimensions.resourceWealth,
            keyResources: resourceData.resources,
            infrastructureControl: composite.dimensions.infrastructureControl,
            policyIndependence: composite.dimensions.policyIndependence,
            currencyStability: composite.dimensions.currencyStability,
